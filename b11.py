...
if __name__ == '__main__':
    try:
        while True:
            print("Среднеарифметическое + чисел. Для остановки работы программы нажмите ctrl+z+Enter или ctrl+c")
            ...
            if n <= 0:
                ...
                continue
            numbers, res = function.arif(n, bot, top) # возврат кортежа из функции
            print('\nСписок ваших сгенерированных чисел в промежутке от [{};{}] : {}'.format(bot, top, numbers))
            print("Среднеарифметическое + чисел : ", "{:.2f}".format(res))
            print('')
